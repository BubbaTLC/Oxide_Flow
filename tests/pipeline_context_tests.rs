...
fn test_pipeline_context_step_references() {
    ...
    let output_data = serde_yaml::from_str(r#"
...
"#).unwrap();
    ...
    metadata.insert("execution_time".to_string(), serde_yaml::Value::Number(250.into()));
    metadata.insert("memory_used".to_string(), serde_yaml::Value::String("128MB".to_string()));
