...
impl Config {
    ...
    pub fn merge_oxi_config(&self, oxi_name: &str, instance_config: &serde_yaml::Value) -> serde_yaml::Value {
...
impl PipelineContext {
    ...
    fn resolve_property_path(&self, step_alias: &str, property_path: &str) -> Result<String, ConfigError> {
        let parts: Vec<&str> = property_path.split('.').collect();
        
        if parts.is_empty() {
            return Err(ConfigError::ValidationError(
                format!("Invalid property path: {}", property_path)
            ));
        }
                ...
                if let Some(value) = metadata.get(parts[1]) {
                    ...
                    return match current_value {
                        ...
                        None => Err(ConfigError::ValidationError(
                            format!("Property path '{}' not found in step metadata '{}'", property_path, step_alias)
                        )),
                    };
                }
            }
        ...
        // Get the step output
        let step_output = self.step_outputs.get(step_alias)
            .ok_or_else(|| ConfigError::ValidationError(
                format!("Step '{}' not found", step_alias)
            ))?;
        ...
        match current_value {
            ...
            None => Err(ConfigError::ValidationError(
                format!("Property path '{}' not found in step '{}'", property_path, step_alias)
            )),
        }
    }
