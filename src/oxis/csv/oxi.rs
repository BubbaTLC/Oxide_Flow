        ...
        if let OxiData::Json(json_value) = result {
            if let serde_json::Value::Array(array) = json_value {
                ...
                if let serde_json::Value::Object(first_row) = &array[0] {
                    assert_eq!(first_row.get("name").unwrap(), &serde_json::Value::String("test".to_string()));
                    assert_eq!(first_row.get("value").unwrap(), &serde_json::Value::Number(serde_json::Number::from(123)));
                }
                
                // Check second row
                if let serde_json::Value::Object(second_row) = &array[1] {
                    assert_eq!(second_row.get("name").unwrap(), &serde_json::Value::String("other".to_string()));
                    assert_eq!(second_row.get("value").unwrap(), &serde_json::Value::Number(serde_json::Number::from(456)));
                }
            } else {
